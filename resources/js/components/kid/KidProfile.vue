    ...
    <div class="profile-card" v-if="!loading">
      <form @submit.prevent="saveProfile" class="profile-form" novalidate>
        <div class="avatar-section">
          <div class="avatar-wrapper" :class="{ empty: !previewAvatar }">
...
export default {
  ...
  methods: {
    async fetchProfile() {
      ...
      try {
        ...
        if (data?.profile) {
          ...
          this.original.avatar_url = data.avatar_url || null;
          this.previewAvatar = this.original.avatar_url;
        }
      } catch (e) {
        this.error = 'Không tải đư��c hồ sơ.';
      } finally {
        this.loading = false;
      }
    },
    onAvatarChange(e) {
      ...
      reader.onload = ev => { this.previewAvatar = ev.target.result; };
      reader.readAsDataURL(file);
    },
    clearAvatar() {
      this.newAvatar = null;
      this.previewAvatar = this.original.avatar_url || null;
    },
    resetForm() {
    ...
    validate() {
      if (!this.form.name) return 'Tên không được để trống';
      return '';
    },
    async saveProfile() {
      this.error = '';
      ...
      try {
        ...
        if (this.newAvatar) fd.append('avatar', this.newAvatar);
        // Use POST (multipart) endpoint
        const { data } = await axios.post('/api/kid/profile/update', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
        if (data?.profile) {
          this.original.name = data.profile.name;
          this.original.avatar_url = data.avatar_url || null;
          this.previewAvatar = this.original.avatar_url;
          this.newAvatar = null;
        }
...
@media (max-width:840px){ .profile-form { grid-template-columns:1fr; } }
.avatar-section { display:flex; flex-direction:column; align-items:center; gap:.6rem; }
.avatar-wrapper { width:180px; height:180px; border:3px solid #d9def2; border-radius:26px; overflow:hidden; background:#f2f5fb; display:flex; align-items:center; justify-content:center; position:relative; }
...
@media (max-width:600px){
  ...
  .profile-card { padding:1rem 1rem 1.3rem; }
  .save-btn, .reset-btn { flex:1; text-align:center; }
}
</style>
